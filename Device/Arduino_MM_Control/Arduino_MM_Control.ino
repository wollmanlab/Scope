...
void PumpHandler(int mode, String data){
    int stepPin = 9;// 2 is stepPin
    int dirPin = 8;// 3 is dirPin
    ...
    for(int x = 0; x < abs(steps); x++) {
      ...
      digitalWrite(5, HIGH);
      //delay(1000);
      delayMicroseconds(wait); // delay between steps, pulse speed
...
void dacHandler(String data){
//  int dac_value = Str2int(data);
  float dac_value = data.toFloat();
  Serial.print("DAC Value: ");
  Serial.println(dac_value);
  float adcValueRead = 0;
  float voltageRead = 0;
  float dac_expected_output;
//  float dac_value = (set_speed/100)*4096;
  dac_expected_output = (5.0/4096.0) * dac_value;
  dac.setVoltage(dac_value,false);
  Serial.print("Expected Voltage: ");
  Serial.println(dac_expected_output);
//    Serial.println(dac_value);
//  delay(250);
